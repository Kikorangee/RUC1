// RUC License Management - Fixed Version
"use strict";

// Define the add-in with explicit parameter separation
geotab.addin.ruc = function(api, state) {
    // Initialize vehicles array
    let vehicles = [];

    // Define the loadRucData function with explicit parameter separation
    const loadRucData = async function() {
        try {
            ...
            for (let i = 0; i < devices.length; i++) {
                const vehicle = devices[i];
                // Use real vehicle properties
                const vehicleName = vehicle.name || 'Unknown Vehicle';
                const serialNumber = vehicle.serialNumber || '--';
                const licensePlate = vehicle.licensePlate || '--';
                
                tableContent += '<tr><td><strong>' + vehicleName + '</strong></td><td>No RUC logs yet</td><td>--</td><td>--</td><td>' + licensePlate + '</td></tr>';
            }
